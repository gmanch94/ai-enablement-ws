...
- **Tier 2 — High-throughput MoE:** DeepSeek-V4-Pro 1.6T MoE (49B active, MIT — successor to V3), GLM-5.2 (MIT), and Mixtral 8x22B (Apache 2.0) for workloads where throughput-per-token-dollar matters more than memory footprint.
- **Tier 3 — Edge / constrained compute:** Phi-4 14B (MIT) and Gemma 4 1B–27B+ (successor to Gemma 3, Gemma Terms of Use) for on-device, latency-sensitive, or cost-constrained scenarios.
...
3. **MoE for throughput** — DeepSeek-V4-Pro's 49B active parameters from a 1.6T parameter MoE delivers frontier quality at tolerable inference cost; MIT license removes legal friction. GLM-5.2 (MIT) offers a second license-clean MoE option for long-horizon agentic workloads.
4. **Phi-4 / Gemma 4 for edge** — Sub-14B models (Gemma 4's smallest tier) with strong reasoning at small scale for local serving, developer laptops (via Ollama/llama.cpp), or embedded inference.
...
- MoE (DeepSeek-V4-Pro, 1.6T total/49B active) requires substantial RAM for CPU offload — 382GB+ was the KTransformers benchmark for the smaller 671B V3 generation; validate current requirements for 1.6T-class MoE before deployment — limits deployment environments
...
4. For MoE (DeepSeek-V4-Pro): validate available RAM before deployment; use KTransformers for commodity hardware, vLLM multi-GPU for production cluster
